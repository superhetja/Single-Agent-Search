	...
	public State getNextState(State s, Action a) {
		...
		switch (a) {
			...
			case TURN_OFF:
				...
				break;
			case TURN_LEFT:
				succState.orientation = ((s.orientation -1+4) % 4);
				break;
			case TURN_RIGHT:
			...
			case GO:
				...
				break;
			case SUCK:
				// maybe check if position in dirt
				succState.dirt.remove(succState.position);
				break;
			default:
				System.out.println("inside default of getNextState()");
				break;
		}

		//System.out.println("In getNextState()  move: " + a + " -> next state: " + succState);
		return succState;
	}
